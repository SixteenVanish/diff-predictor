...
# 参照ReSSL设置
# 整理了diffusion predictor模块后复现baseline
work_dir = "./work_dirs/tiny/byol/byol_resnet18_1xb256-coslr-200e_tinyimagenet_v4"
...
model = dict(
    ...
    loss_byol_cos=1,
    crop_cfg = dict(
        num_crops=num_crops,    # default:2. If num_crops is not 2, then train_pipeline need to be changed.
...
default_hooks = dict(checkpoint=dict(interval=10, max_keep_ckpts=20))
...
train_pipeline = [
    ...
    dict(type='MultiView', num_views=num_crops, transforms=[view_pipeline]),
