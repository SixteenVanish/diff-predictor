...
def fill_missing_settings(cfg, default_cfg):
    for key, value in default_cfg.items():
        # 如果 cfg 中缺少 default_cfg 中的某个键，将其添加到 cfg 中
        if key not in cfg:
            if isinstance(value, ConfigDict):
                cfg[key] = ConfigDict()
                fill_missing_settings(cfg[key], value)
            else:
                cfg[key] = value
        # 如果 cfg 中存在与 default_cfg 相应键对应的 ConfigDict 类型的值，
        # 则递归填充缺失的设置
        elif isinstance(value, ConfigDict) and isinstance(cfg[key], ConfigDict):
            fill_missing_settings(cfg[key], value)
...
class DDIMScheduler():
    ...
    def _get_variance(self, timestep, prev_timestep):
        ...
        alpha_prod_t_prev = self.alphas_cumprod[prev_timestep] if prev_timestep[0] >= 0 else self.final_alpha_cumprod.expand(prev_timestep.shape[0])
        alpha_prod_t_prev = alpha_prod_t_prev.to(alpha_prod_t.device)
    ...
    def step(
        ...
        prev_timestep = prev_timestep.to(model_output.device)
        ...
        if use_clipped_model_output:
            ...
            model_output = (sample - torch.einsum("b,bd->bd", alpha_prod_t ** (0.5), pred_original_sample)) / beta_prod_t.view(-1, 1) ** (0.5)

        # 6. compute "direction pointing to x_t" of formula (12) from https://arxiv.org/pdf/2010.02502.pdf
        # pred_sample_direction = (1 - alpha_prod_t_prev - std_dev_t**2) ** (0.5) * model_output
        pred_sample_direction = ((1 - alpha_prod_t_prev - std_dev_t**2) ** (0.5)).view(-1, 1) * model_output

        # 7. compute x_t without "random noise" of formula (12) from https://arxiv.org/pdf/2010.02502.pdf
        # prev_sample = alpha_prod_t_prev ** (0.5) * pred_original_sample + pred_sample_direction
        prev_sample = (alpha_prod_t_prev ** (0.5)).view(-1, 1) * pred_original_sample + pred_sample_direction
...
class DDIMPipeline:
    ...
    def __call__(
        ...
        # set step values
        if infer_range == [0.5, 1]:
            self.scheduler.set_timesteps(num_inference_steps*2)
            inference_range = self.scheduler.timesteps[len(self.scheduler.timesteps)//2:]
            print('inference_range', inference_range)
        elif infer_range == [0, 1]:
            self.scheduler.set_timesteps(num_inference_steps)
            inference_range = self.scheduler.timesteps

        for t in inference_range:
            bsz, dim = image.shape
            t = t.repeat(bsz)
            time_emb = get_timestep_embedding(timesteps=t, embedding_dim=dim)
            predicted_embedding, _ = self.model(image, condition, time_emb.to(device))

            # 2. predict previous mean of image x_t-1 and add variance depending on eta
            # eta corresponds to η in paper and should be between [0, 1]
            # do x_t -> x_t-1
            image = self.scheduler.step(
                predicted_embedding, t, image, eta=eta, use_clipped_model_output=True, generator=generator
            )['prev_sample'] 
