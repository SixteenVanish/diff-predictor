...
class DiffBYOL(BaseSelfSupervisor):
    def __init__(self,
        ...
        if self.loss_byol_cos > 0 or self.loss_byol_mse > 0:
        ...
        if self.diff_pred:
            ...
            if self.diff_cfg.cat_or_add == 'cat':
                if not self.diff_cfg.remove_condition_T:
                    dim_in = self.diff_cfg.model.in_channels * 3
                else:
                    dim_in = self.diff_cfg.model.in_channels * 2
            else:
            ...
            if self.diff_cfg.model.type == 'MLP':
                ...
                Transformer(
                    ...
                    double_self_attention = self.diff_cfg.model.trans_params.double_self_attention,
    ...
    def loss(self, inputs: List[torch.Tensor], data_samples: List[DataSample],
        ...
        if self.diff_pred:
            ...
            for latents, condition_feat in latent_condition:
                ...
                predicted_embedding = self.DMmodel(noisy_latents, condition_feat, timesteps, self.diff_cfg.cat_or_add, self.diff_cfg.remove_condition_T)
...
class Transformer(nn.Module):
    def __init__(
        ...
        self.proj_in = nn.Linear(dim_in, self.inner_dim)
        self.norm_in = None
        ...
        self.proj_to_clip_embeddings = nn.Linear(self.inner_dim, dim_out)
        
    def forward(self, noisy_latents, condition_feat, timesteps, cat_or_add, remove_condition_T):  # 前向传播函数
        bsz, dim = noisy_latents.shape
        ...
        if cat_or_add == 'add':
            ...
            if not remove_condition_T:
                ...
                hidden_states = torch.cat((noisy_latents, time_emb), dim=1)  # [bs, 2*dim] = [bs, dim_in]
            
        if hidden_states.ndim == 5:  # 如果输入是5维张量（例如，来自卷积神经网络的特征图）
            hidden_states = hidden_states.permute((0, 2, 3, 4, 1))  # 重排维度顺序，使通道维度在最后
        if self.flatten:  # 如果需要在MLP前将输入展平
            hidden_states = hidden_states.reshape(-1, hidden_states.shape[-1])  # 将输入展平为二维张量

        hidden_states = self.proj_in(hidden_states)     # [bs, dim_in] -> [bs, inner_dim]
        hidden_states = hidden_states.view(bsz, -1, self.inner_dim)    # [bs, inner_dim] -> [bs, 1or3, inner_dim]
        if self.norm_in is not None:
            hidden_states = self.norm_in(hidden_states)
        for block in self.transformer_blocks:
            hidden_states = block(hidden_states, attention_mask=None)   # [bsz, 1, inner_dim]
        hidden_states = self.norm_out(hidden_states)
        predicted_image_embedding = self.proj_to_clip_embeddings(hidden_states).view(bsz, self.dim_out)
        
        return predicted_image_embedding  # 通过MLP进行前向传播并返回结果
