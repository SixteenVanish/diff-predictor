...
class DiffBYOLv0(BaseSelfSupervisor):
    def __init__(self,
                 backbone: dict,
                 neck: dict,
                 head: dict,
        ...
        if self.loss_byol_cos > 0 or self.loss_byol_mse > 0 or self.loss_add_cos > 0:
            ...
            for proj_target, proj_online in projs_pairs:
                ...
                if self.loss_add_cos:
                    ...
                    if self.diff_cfg.distT == 'gaussian':
                        Tmax = self.scheduler.config.num_train_timesteps
                        mu = Tmax / 2
                        gaussian_dist = dist.Normal(mu, 1)
                        timesteps = torch.clamp(torch.round(gaussian_dist.sample(sample_shape=(bsz,))), 0, Tmax).long().to(latents.device)                    
                    elif self.diff_cfg.distT == 'inc':
                        Tmax = self.scheduler.config.num_train_timesteps
                        probs = torch.arange(0, Tmax)  # 0,...,Tmax-1
                        linear_inc_dist = dist.Categorical(probs)
                        timesteps = torch.clamp(linear_inc_dist.sample(sample_shape=(bsz,)), 0, Tmax).to(latents.device) 
                    elif self.diff_cfg.distT == 'uniform':
        ...
        if self.diff_pred and not self.loss_add_cos:
            ...
            for latents, condition_feat in latent_condition:
                ...
                if self.diff_cfg.distT == 'gaussian':
                    ...
                    probs = torch.arange(0, Tmax)   # 0,...,Tmax-1
                    linear_inc_dist = dist.Categorical(probs)
                    timesteps = linear_inc_dist.sample(sample_shape=(bsz,)).to(latents.device)
                elif self.diff_cfg.distT == 'uniform':
                    timesteps = torch.randint(low=0,
                                            high=self.scheduler.config.num_train_timesteps,
                                            size=(bsz,),
...
# 没有cross，没有proj_to_clip_embeddings，且cat，则维度会出问题
