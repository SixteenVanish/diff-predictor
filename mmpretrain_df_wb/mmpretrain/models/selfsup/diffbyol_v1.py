...
class DiffBYOL(BaseSelfSupervisor):
    def __init__(self,
        ...
        if self.decoder_cfg is not None:
            self.decoder_online = Decoder(img_size=decoder_cfg.online_img_size, cfg=decoder_cfg) if decoder_cfg.online_img_size != 0 else None
            self.decoder_target = Decoder(img_size=decoder_cfg.target_img_size, cfg=decoder_cfg) if decoder_cfg.target_img_size != 0 else None
            self.decoder_predictormid = Decoder(img_size=decoder_cfg.predictor_img_size, cfg=decoder_cfg) if decoder_cfg.predictor_img_size != 0 else None
    ...
    def loss(self, inputs: List[torch.Tensor], data_samples: List[DataSample],
        ...
        if self.loss_add_cos > 0:
            ...
            for proj_target, proj_online in projs_target_online:
                ...
                target = proj_target.detach()
                pred_part2 = self.DMmodel([[proj_target, proj_online]])['diff_pred_target_list'][0]
        ...
        if self.decoder_cfg is not None:
            ...
            if self.decoder_cfg.decoder_layer == "neck":
                decoder_online_view_zip = zip(projs_online[:global_num], views_online[:global_num])
                decoder_target_view_zip = zip(projs_target[:global_num], views_target[:global_num])
            elif self.decoder_cfg.decoder_layer == "backbone":
                decoder_online_view_zip = zip(maps_online[:global_num], views_online[:global_num])
                decoder_target_view_zip = zip(maps_target[:global_num], views_target[:global_num])
            elif self.decoder_cfg.decoder_layer == "predictor_mid":
                projs_predictormid = diff_output['middle_embedding_list'] if diff_output is not None else byol_output['middle_embedding_list']
                decoder_predictormid_view_zip = zip(projs_predictormid[:global_num], views_online[:global_num])
            elif self.decoder_cfg.decoder_layer == "predictor":
                projs_predictor = diff_output['diff_pred_target_list'] if diff_output is not None else byol_output['diff_pred_target_list']
                decoder_predictormid_view_zip = zip(projs_predictor[:global_num], views_online[:global_num])
            
            if self.decoder_online is not None:
                losses['loss_decoder_online'] = 0
                for proj_online, img in decoder_online_view_zip:
            ...
            if self.decoder_target is not None:
                losses['loss_decoder_target'] = 0
                for proj_target, img in decoder_target_view_zip:
            ...
            if self.decoder_predictormid is not None:
                losses['loss_decoder_predictormid'] = 0
                for proj_predictormid, img in decoder_predictormid_view_zip:
                    proj_predictormid = proj_predictormid.detach()
                    img = img.detach()
                    image_predictormid = self.decoder_predictormid(proj_predictormid)
                    losses['loss_decoder_predictormid'] += F.mse_loss(image_predictormid.float(), img.float(), reduction="mean")
