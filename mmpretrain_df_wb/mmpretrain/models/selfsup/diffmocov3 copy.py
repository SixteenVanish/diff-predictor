    ...
    def loss(self, inputs: List[torch.Tensor], data_samples: List[DataSample],
        ...
        if self.loss_original > 0:
            ...
            for proj_target, proj_online in projs_target_online:
                ...
                target = nn.functional.normalize(proj_target, dim=1)
                ...
                losses['loss_original'] += self.loss_module(logits, labels) * self.loss_original
            losses['loss_original'] /= (num_pairs/2)
            
        elif self.diff_pred:
            assert self.DMmodel is not None
            output_both, output_loss, output_pred= False, False, False
            ...
            if self.diff_cfg is not None:
                for key, value in self.diff_cfg.loss.items():
                    if value > 0:
                        output_both = True
                        break
            if output_loss and output_pred:
                output_both = True
            
            if output_both:
                pred, diff_losses = self.DMmodel(projs_target_online, output_both=output_both, output_pred=output_pred)
                pred = pred[0]
                ...
                for proj_target, proj_online in projs_target_online:
                    ...
                    if output_both:
                        pred, diff_losses = self.DMmodel([[proj_target, proj_online]], output_both=output_both, output_pred=output_pred)
                        pred = pred[0]
                        losses.update(diff_losses)
                    else:
                        pred = self.DMmodel([[proj_target, proj_online]], output_pred=output_pred, output_both=output_both)[0]
                
                def compute_loss_diff_CE(pred, proj_target):
                    # predictor computation
                    pred = pred[0]
                    # normalize
                    pred = nn.functional.normalize(pred, dim=1)
                    ...
                    return 
                    if self.loss_diff_CE > 0:
                        losses['loss_diff_CE'] += self.loss_module(logits, labels) * self.loss_diff_CE
