...
def parse_args():
    ...
    parser.add_argument(
        '--extract-stage',
        choices=['backbone', 'neck', 'DM', 'pre_logits', 'target_net'],
...
def main():
    ...
    for data in progress.track(dataloader, description='test...'):
        with torch.no_grad():     
            ...
            if args.decode:
                # decode
                if args.extract_stage == "target_net":
                    batch_images_online = model.decoder_target(batch_features)
                else:
                    batch_images_online = model.decoder_online(batch_features)

                # visualization
                bsz = batch_images_online.shape[0]
                # bsz = 2
                if False:
                ...
                if True:   # 使用 make_grid
                    for idx in range(0, bsz):
                        ...
                        images = make_grid([data_input.astype('uint8'), image_online.astype('uint8')], ['transformed', 'decoded'])
                        pic_name = f"{file_name}_label{gt_label}_{idx}.png"
                        visualizer.visualize_cls(
                            images,
                            data_sample,
                            # name=f"decoded_{idx}.png",
                            # out_file=test_work_dir + f"decoded_{idx}.png",
                            name=pic_name,
