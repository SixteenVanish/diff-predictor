...
def parse_args():
    ...
    parser.add_argument(
        '--vis-stage',
        choices=['backbone', 'neck', 'DM', 'pre_logits'],
    ...
    # add:
    parser.add_argument('--DMtimes', type=int, default=5)
    parser.add_argument('--DMstep', type=int, default=0)
    # parser.add_argument('--DMstep', type=List[int], default=[0])
    parser.add_argument('--croptype', type=str, default='RandomResizedCrop')
...
def main():
    ...
    for data in progress.track(dataloader, description='Calculating...'):
        with torch.no_grad():
            ...
            #print("class ", str(int(batch_labels[0])))
            # extract backbone features
            extract_args = {}
            if args.vis_stage:
                extract_args['stage'] = args.vis_stage
            if args.DMtimes:
                extract_args['DMtimes'] = args.DMtimes
            if args.DMstep:
                extract_args['DMstep'] = args.DMstep
            # if args.croptype == 'FiveCrop':
            #     extract_args['fivecrop'] = True
            batch_features = model.extract_feat(batch_inputs, **extract_args)
            #model.distributionAnalysis(batch_features)
            # post process
    ...
    for i in range(len(features[0])):   # features作为list装载多个batch的数据，每个batch的数据是一个tuple，这个tuple的维度取决于out_indices/DMtimes，tuple中的每个元素为最终特征(bsz, dim)
        if not args.vis_stage or args.vis_stage=='backbone':
            args.vis_stage = 'backbone'
            key = f"feat_{args.vis_stage}_{args.max_num_class}classes_{args.croptype}{args.save_suffix}"
            #key = 'feat_' + args.vis_stage + '_' + str(model.backbone.out_indices[i])
            results[key] = np.concatenate(
                [batch[i].cpu().numpy() for batch in features], axis=0)
        elif args.vis_stage=='neck':
            key = f"feat_{args.vis_stage}_{args.max_num_class}classes_{args.croptype}{args.save_suffix}"
            results[key] = np.concatenate(
                [batch[i].cpu().numpy() for batch in features], axis=0)
        elif args.vis_stage=='DM':
            break
    ...
    if args.vis_stage=='DM':
        # original: len(features)=N, len(features[0])=DMtimes, features[0][0].shape=[bsz, dim], len(labels)=N*bsz
        # after change: features_.shape=[N, bsz, DMtimes, dim], len(labels_)=N*bsz*DMtimes
        key = f"feat_{args.vis_stage}_{args.max_num_class}classes_step{args.DMstep}_{args.croptype}{args.save_suffix}"
        if args.pca:
            key = 'PCA' + key
        else:
            key = 'TSNE' + key
        
        features_ = torch.stack([torch.stack(batch) for batch in features]) # [N, DMtimes, bsz, dim]
        N, DMtimes, bsz, dim = features_.shape
        features_ = features_.permute(0,2,1,3)  # [N, bsz, DMtimes, dim]
        results[key] = features_.reshape((N*bsz*DMtimes, dim)).cpu().numpy()
        labels = np.repeat(labels, args.DMtimes).tolist()
    ...
    # build t-SNE model
    if args.pca:
        fit_model = PCA(n_components=args.n_components)
    else:
        fit_model = TSNE(
    ...
    for key, val in results.items():
        result = fit_model.fit_transform(val)
